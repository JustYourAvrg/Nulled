        ...
        try:
            if self.checks() == True:
                ...
                if self.data['webhook_url'] == '':
                    self.ui.after(0, self.popup.showinfo('Webhook URL Empty | Webhook will not be sent |'))
    ...
    def start_func(self):
        try:
            if not self.start_flag and not self.end_flag:
                ...
                with open(os.path.join(pathlib.Path(__file__).parent.resolve(), 'logs.log'), 'w') as f:
                    f.write('')

                self.helper.activate_roblox_window()
                self.init.start()
            self.start_flag = False
