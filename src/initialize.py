...
from src.ui.popups import showmsg
...
class Initialize:
    ...
    def start(self):
        try:
            ...
            self.webhook.send_embed(title="**Macro Started!**", text="### Macro has been started")
            self.data = self.helpers.load_config()
            time.sleep(1)
            ...
            if self.data['auto_merchant']['enabled'] == 1:
                ...
                if check_for_tesseract_ocr == 'Not Found':
                    showmsg(title="ERROR", message="TESSERACT-OCR NOT FOUND PLEASE DOWNLOAD IT TO USE AUTO MERCHANT")
    ...
    def auto_potion_crafting(self):
        try:
            ...
            showmsg(title='ERROR', message='No potion is enabled, please enable a potion to start crafting!')
            return
        ...
        try:
            ...
            while not self.auto_potion_crafting_event.is_set():
                with self.lock:
                    logs.handle_log_msg('INFO', f"ATTEMPTING TO CRAFT POTION: {self.cur_potion}")
        ...
        try:
            while not self.biome_detection_event.is_set():
                # with self.lock:
                    ...
                    if detected_biome == self.current_biome:
                        pass
            ...
            while not self.aura_clipping_event.is_set():
                # with self.lock:
                    try:
                        ...
                        time.sleep(0.5)
                        if last_equipped_aura == self.cur_last_aura:
                            pass
    ...
    def auto_biome_randomizer(self):
        ...
        try:
            while not self.biome_randomizer_event.is_set() and self.current_biome not in ["GLITCHED", "DREAMSPACE"]:
                with self.lock:
    ...
    def auto_strange_controller(self):
        ...
        try:
            while not self.strange_controller_event.is_set() and self.current_biome not in ["GLITCHED", "DREAMSPACE"]:
                with self.lock:
