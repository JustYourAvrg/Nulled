...
class UI(ctk.CTk):
    ...
    def save_ps_text(self, event):
        ...
        choice = askyesno(title="Confirmation", message=f'Are you sure you would like to set "{url}" as your private server link?')
        if choice:
            self.helper.edit_config(['private_server_link'], url)
            return
        showmsg("INFO", "PRIVATE SERVER LINK NOT ADDED")


    def save_webhook_text(self, event):
        url = self.webhook_entry.get()
        
        choice = askyesno(title="Confirmation", message=f'Are you sure you would like to set "{url}" as your webhook link?')
        if choice:
            self.helper.edit_config(['webhook_url'], url)
            return
        showmsg("INFO", "WEBHOOK LINK NOT ADDED")
