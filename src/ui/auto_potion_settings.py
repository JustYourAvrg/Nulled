...
class APS:
    ...
    def toggle_potion_enabled(self, potion, checkbox: ctk.CTkSwitch):
        new_val = checkbox.get()
        
        self.helper.edit_config(['auto_potion', 'potions', potion, 'enabled'], new_val)


    def enable_auto_potion(self, checkbox: ctk.CTkSwitch):
        val = checkbox.get()
    ...
    def auto_potion_settings_ui(self):
        ...
        toggle_potion_frame.grid_columnconfigure(0, weight=1)
        toggle_potion_frame.grid_propagate(0)

        for idx, potion in enumerate(self.data['auto_potion']['potions']):

            potion_checkbox = ctk.CTkSwitch(toggle_potion_frame, text=potion, fg_color='#1c1c1c', progress_color="#3c3c3c", button_color="#c4c4c4")
            potion_checkbox.grid(row=idx, column=0, padx=5, pady=5, sticky='w')
        ...
        enable_auto_potion = ctk.CTkSwitch(potion_settings_frame, text="Auto Potion", fg_color='#1c1c1c', progress_color="#3c3c3c", button_color="#c4c4c4")
