...
class HelperFunctions:
    ...
    def link_checks(self):
        self.config_data = self.load_config()
        webhook_url = self.config_data['webhook_url']
        
        handle_log_msg('INFO', f'Link checks: {webhook_url}')
        if webhook_url == '':
            return False
        return True
    ...
    def update(self):
        try:
            version_file_path = os.path.join(pathlib.Path(__file__).parents[2].resolve(), 'version.json')
            ...
            if response.status_code == 200:
                ...
                if version_data['version'] != tag_name:
                    choice = self.popup.askconfirmation(f"A New update has been detected {tag_name} would you like to update?")
                    if choice:
